...
def _user(client: TestClient, prefix: str) -> tuple[str, dict]:
    ...
    assert client.post("/api/auth/register", json={"username": name, "password": "secret123"}).status_code == 201
    token = client.post("/api/auth/login", json={"username": name, "password": "secret123"}).json()["access_token"]
...
def _character(client: TestClient, headers: dict, name: str) -> str:
    ...
    response = client.put(
        ...
        headers=headers,
        json={"name": name, "bio": "擅长追踪", "ability_asset_ids": [ability["id"]], "lock_version": 1},
    )
...
def _promote(name: str) -> None:
    with psycopg.connect(os.environ["DATABASE_URL"].replace("+asyncpg", "")) as db:
        db.execute("UPDATE users SET is_admin=TRUE WHERE username=%s", (name,))
        db.commit()
...
def test_player_roster_is_public_and_author_trial_is_excluded_from_stats():
    with TestClient(app) as client, patch("app.api.routes.scenario_domain._prepare_scenario_challenge", _no_background_work):
        admin_name, admin = _user(client, "scadm")
        _promote(admin_name)
        admin = {"Authorization": admin["Authorization"]}
        scenario = client.post("/api/admin/scenarios", headers=admin, json={"name": "追捕", "summary": "一场追捕", "background": "城门初闭", "victory_condition": "逃出城门"})
        ...
        roster = client.post(f"/api/creator/scenarios/{scenario.json()['id']}/rosters", headers=author, json={"character_asset_id": character_id, "guidance": "沿着脚印追踪"})
        ...
        challenge = client.post(f"/api/scenario-rosters/{roster.json()['id']}/challenges", headers=author, json={"character_asset_id": character_id})
...
def test_challenge_prepares_before_actions_and_accepts_action_asynchronously():
    with TestClient(app) as client, patch("app.api.routes.scenario_domain._prepare_scenario_challenge", _no_background_work), patch("app.api.routes.scenario_domain._resolve_scenario_action", _no_background_work):
        admin_name, admin = _user(client, "streamadm")
        _promote(admin_name)
        scenario = client.post("/api/admin/scenarios", headers=admin, json={"name": "流式追捕", "summary": "一场追捕", "background": "城门初闭", "victory_condition": "逃出城门"}).json()
        ...
        roster = client.post(f"/api/creator/scenarios/{scenario['id']}/rosters", headers=author, json={"character_asset_id": character_id, "guidance": "沿着脚印追踪"}).json()
        challenge = client.post(f"/api/scenario-rosters/{roster['id']}/challenges", headers=author, json={"character_asset_id": character_id}).json()
...
def test_roster_detail_scopes_history_and_progress_to_viewer():
    with TestClient(app) as client, patch("app.api.routes.scenario_domain._prepare_scenario_challenge", _no_background_work):
        ...
        _promote(admin_name)
        scenario = client.post("/api/admin/scenarios", headers=admin, json={"name": "详情追捕", "summary": "一场追捕", "background": "城门初闭", "victory_condition": "逃出城门"}).json()
        assert client.post(f"/api/admin/scenarios/{scenario['id']}/publish", headers=admin).status_code == 200
        ...
        roster = client.post(f"/api/creator/scenarios/{scenario['id']}/rosters", headers=author, json={"character_asset_id": author_character, "guidance": "沿脚印追踪"}).json()
        run = client.post(f"/api/scenario-rosters/{roster['id']}/challenges", headers=challenger, json={"character_asset_id": challenger_character}).json()

...
推演（_build_deduce_llm）与转写（_build_transcribe_chain）、猜词配对/检定（_build_pair/verify_llm）
在此按测试作用域打桩。
"""
...
def test_rematch_copies_snapshot_and_guess_state():
    ...
    with TestClient(app) as client:
        ...
        with (
            ...
            g = client.post(f"/api/battles/{orig['id']}/guess", json={"text": "控制重力"}, headers=h_b)
        ...
        # B 视角：快照奇术一致 + 猜词进度带入（次数/历史保留、可续猜）
...
def test_draw_guess_one_crack_wins_and_rank_recalc():
    ...
    with TestClient(app) as client:
        ...
        # A 一次全破 B 的血咒（配对命中 + 检定猜出）→ 本行翻转；B 未收手前不结算
        pair, verify = _guess_pipeline(lambda kw: PairMatch(snippet="以自身鲜血为引发动诅咒"), True)
        with (
            patch("app.services.battle._build_pair_llm", return_value=pair),
            patch("app.services.battle._build_verify_llm", return_value=verify),
        ):
            g = client.post(f"/api/battles/{b['id']}/guess", json={"text": "以血引咒反噬"}, headers=h_a)
            assert g.status_code == 202
            ga = _wait_guess(client, b["id"], h_a, attempts_before=0)
...
def test_draw_guess_both_crack_keeps_draw():
    ...
    with TestClient(app) as client:
        ...
        # A 全破 B 的血咒
        pair_a, verify = _guess_pipeline(lambda kw: PairMatch(snippet="血咒"), True)
        with (
            patch("app.services.battle._build_pair_llm", return_value=pair_a),
            patch("app.services.battle._build_verify_llm", return_value=verify),
        ):
            assert client.post(f"/api/battles/{b['id']}/guess", json={"text": "血咒"}, headers=h_a).status_code == 202
            _wait_guess(client, b["id"], h_a, attempts_before=0)

        # B 全破 A 的影刃
        pair_b, verify = _guess_pipeline(lambda kw: PairMatch(snippet="影刃"), True)
        with (
            patch("app.services.battle._build_pair_llm", return_value=pair_b),
            patch("app.services.battle._build_verify_llm", return_value=verify),
        ):
            assert client.post(f"/api/battles/{b['id']}/guess", json={"text": "影刃"}, headers=h_b).status_code == 202

"""Drop the retired character-comparison cache table (superseded by Redis-backed pair comparison)."""
...
def upgrade() -> None:
    op.execute(sa.text('DROP TABLE IF EXISTS "character_comparisons" CASCADE'))


def downgrade() -> None:
    # Retired cache data is intentionally not reconstructed; restore the empty table for tooling.
    op.create_table(

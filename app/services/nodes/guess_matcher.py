...
**从严判定（防放水）**：只有用户猜测**明确、具体**地指向该能力某个实际要素才算命中——说出的是这门
奇术特有的机制/效果/限制/发动方式，换了别的能力就对不上。含糊宽泛、放到多数能力上都成立的通用说法
（如"很强"、"有攻击手段"、"能远程"）不算有价值线索，不得输出；只把已有线索换种说法重述的，也不算新线索。

你需要全面对比用户的猜测文本与该能力的实际信息。**只有在确认命中确凿、有价值的新线索时才输出片段，
否则 snippet 留空——拿不准是否真正命中、是否与已有线索重复、是否只是擦边时，一律不输出，绝不放水。**
命中的片段须兼顾
...
3.新增性：必须揭示该能力此前尚未解锁的一个具体要素，不能只是把已有线索换个说法重述
"""
...
判定规则：
- 必须覆盖该能力**自身具备的全部重要要素**才算猜出：核心机制、主要效果，以及它实际有关的关键限制与发动条件。缺任何一个重要要素（尤其限制/代价/触发禁忌这类关键设计）即判未猜出。
- 覆盖须与该能力真实机制**严谨对应**：线索须准确、具体地指向其实际机制；宽泛含糊、能套用到多数能力的通用说法不算覆盖，也不能据此判猜出。
- 线索只擦边或与真实机制相悖的不计入覆盖；语义相同的转述可以接受。
- 判为猜出时，理由须逐条说明覆盖了哪些重要要素；判未猜出时须明确点出还缺什么。
一项完整的能力可以由以下要素构成：**限制**（外部的发动条件、代价、触发禁忌）、**发动方式**（如何使出）、
**效果**（做什么/对目标产生什么）、**达成效果的方式**（通过什么手段实现该效果）、
**表征**（外在呈现：外观、颜色、声音、场面）、**其他限制参数**（比如范围，持续时间，冷却等相关内容）。
以上几个所谓的要素仅作参考，实际需要你结合具体的能力本身做判断。
